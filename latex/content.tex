...
% TODO Maybe Add communication overhead here, too

    \item We present additional algorithms from the area of \textit{routing}
    and \textit{topology building}

    % TODO Add quantifiable point, e.g. number of algorithms presented and what
    % we derived from it We present 14 algorithms from the field of data
    % sampling and 5 related algorithms, i.e. from routing and topology
    % control. We additionally present 2 algorithms which combine techiques
    % from sampling and filtering.
\end{enumerate}
...
% TODO EAST. Algorithm basics. Uses a clustering algorithm in which the sensing
% area is devided dynamically into cells. Cells consist of a coordinator
% (cluster head), member and representative sensor nodes. Member nodes do not
% send their data, representative nodes send their sampled values to the
% coordinator only if 
