...
\begin{enumerate}
    ...
    % We present 14 algorithms from the field of data sampling and 5 related algorithms, i.e. from routing and topology control
\end{enumerate}
...
% We additionally split the subclasses into additional classes, or
% subsubclasses. Algorithms can be in multiple subsubclasses.
...
% consists of a mac header and a payload (data)
    ...
    \item[Stream Sampling, Reservoir Sampling]
        ...
        % QUESTION the sources refers to reservoir sampling as an ealy stream
        % sampling algorithm.
...
% TODO: Reasoning for classifying in such a way. Justify
% thesis title and why filtering algorithms are included.
...
% TODO add two other references of papers which are not in the category
% adaptive sampling but use a sampling scheme
...
% TODO make a different introduction if you dont find a paper which could
% introduce cdg or a paper which focusses on small scale SNs
...
standard deviation $ \sigma $ was $ < 0.15°C $. % Reread that, as in stcdg,
...
synchronized prediciton models with the sink nodes. Such a structure is
...
% often not fesiable to run on energy constraint nodes source: Energy-Efﬁcient
...
% view, tree topologies in which nodes closer to the sink/base station have
% more child nodes (i.e. connections to other sensor for which they route the
...
% % EDAL is actually a routing scheme...
% The work of Yao et al.~\cite{yao2015edal} also leverages multiple techniques to
% achieve efficient data collection in sensor networks. The algortihm,
% \textit{EDAL}, utilizes techniques from operational research to find the best
% routes in a large-scale \ac{WSN} in terms of energy consumption and message
% delay. Afterwards, compressive sampling is employed to reduce the amount of
% required samples to reconstruct the original signal at the sink with high
% probability.
...
In this section, we will present possible combinations of algorithms presented
...
% TODO combinations with routing and topology control or only with algorithms
% from taxonomy?
...
% Talk about the big picture (i.e. how IoT will require efficient algorithms to
% reduce the amount of data to be processed). Talk about how many presented
% algorithms focus on WSNs, however, the energy consumption goal in WSNs is
% proportional to general network utilization. Talk about the general tradeoff
% of ressources, i.e. energy vs data granularity, accuracy, latency. Sum up
% what was present in the paper. Talk about that each how each algorithm can
% generally be combined with processes from another one (i.e. sampling and
% filering) and that there is room for more research in those fields. Maybe
% some kind of outlook which fields need more attention in terms of research?.
...
% Idea: Talk about how many techniques in WSNs focus on reducing energy
% expenditure and how this is often a trade off between accuracy and energy
% consumption.